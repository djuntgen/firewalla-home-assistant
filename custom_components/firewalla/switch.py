...
async def async_setup_entry(
    ...
    _LOGGER.debug(
        "Setting up Firewalla rule control switch platform for entry %s",
    ...
    try:
            ...
            for rule_id, rule_data in rules.items():
                try:
                    ...
                    if not isinstance(rule_data, dict):
                        _LOGGER.warning(
                            "Invalid rule data for %s: %s", rule_id, type(rule_data)
                        )
                        continue
                    ...
                    _LOGGER.error(
                        "Error creating switch entity for rule %s: %s", rule_id, err
                    )
                    continue
        else:
        ...
        if entities:
            ...
            _LOGGER.info(
                "Successfully added %d Firewalla rule control switch entities",
                len(entities),
            )
        else:
        ...
        _LOGGER.exception(
            "Unexpected error setting up Firewalla rule control switch platform: %s",
            err,
        )
        raise HomeAssistantError(
            f"Failed to set up Firewalla rule control switch platform: {err}"
        ) from err
...
class FirewallaRuleSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def extra_state_attributes(self) -> Dict[str, Any]:
        ...
        for attr_key in RULE_ATTRIBUTES:
            if attr_key in current_rule_data:
                ...
                if attr_key in ["created_at", "modified_at"] and isinstance(
                    value, (int, float)
                ):
                    try:
        ...
        attributes["rule_status"] = (
            "active" if not current_rule_data.get("paused", False) else "paused"
        )
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Unexpected error turning on rule %s: %s", self._rule_id, err
            )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Unexpected error turning off rule %s: %s", self._rule_id, err
            )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.debug(
            "Rule switch entity added to hass: %s (%s)", self._rule_id, self.name
        )
