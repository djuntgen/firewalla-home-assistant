...
class TestAPIErrorHandling:
    ...
    async def test_auth_refresh_failure(self, client, mock_aiohttp_session):
        ...
        with pytest.raises(
            ConfigEntryAuthFailed, match="MSP API authentication refresh failed"
        ):
            await client._make_request("GET", "/test/endpoint")
...
class TestCoordinatorErrorHandling:
    ...
    async def test_update_data_partial_failure(self, coordinator, mock_api_responses):
        ...
        coordinator.api.get_box_info = AsyncMock(
            return_value=mock_api_responses["box_info"]
        )
        coordinator.api.get_devices = AsyncMock(
            side_effect=HomeAssistantError("Devices API failed")
        )
...
class TestEntityErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_switch_turn_on_api_failure(
        self, mock_coordinator, mock_devices_data
    ):
        ...
        with patch.object(switch, "is_on", False), patch.object(
            switch, "_find_paused_block_rule", return_value=None
        ):

            with pytest.raises(
                HomeAssistantError, match="Failed to enable internet blocking"
            ):
